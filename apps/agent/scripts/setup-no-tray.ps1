...
.EXAMPLE
    ...
    # One-shot
    .\setup-no-tray.ps1 -Action install
#>
...
$BootstrapUrl = "https://cdn.jsdelivr.net/gh/calirko/backupr@main/apps/agent/scripts/setup-no-tray.ps1"
...
function Write-Banner {
    ...
    Write-Host "${Gray}  Agent Installer & Service Manager (no tray)${Reset}"
    # Write-Host ""
}
...
function Action-Setup {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host ""
        Write-Host "  Setup complete. Start the service with: .\setup-no-tray.ps1 -Action start" -ForegroundColor Cyan
    } else {
        Write-Host ""
